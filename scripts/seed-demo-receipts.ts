...
interface SeedScenario {
  ...
  sku: string;
  strictness?: number;
  expectedDecision: 'pass' | 'step_up' | 'decline';
  description: string;
}

// 15 SKU catalog items for diverse checkout generation
const CATALOG_ITEMS: Record<string, any> = {
  SKU_ESP_13200: {
    sku: 'SKU_ESP_13200',
...
function buildCart(skuKey: string) {
  const item = CATALOG_ITEMS[skuKey];
  if (!item) throw new Error(`Unknown skuKey: ${skuKey}`);
  return {
...
async function main() {
  ...
  for (let i = 0; i < SCENARIOS.length; i++) {
    ...
    try {
      ...
      const dec = data.decision as 'pass' | 'step_up' | 'decline';
      decisionCounts[dec] = (decisionCounts[dec] || 0) + 1;
      successCount++;

      const mark = dec === sc.expectedDecision ? '✓' : '~';
      console.log(
        `  ${mark} [${(i + 1).toString().padStart(2, ' ')}/${SCENARIOS.length}] ${dec.toUpperCase().padEnd(7)} | ${sc.description}`
      );
    } catch (err: any) {
